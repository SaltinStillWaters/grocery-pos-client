<template>
  <v-card elevation="0" class="rounded-lg">
    <v-card-title class="d-flex align-center px-4 py-3 bg-grey-lighten-4 border-bottom">
      <v-icon
    ...
    <v-card-text class="pa-0">
      ...
      <div class="px-5 py-4 bg-white">
        <v-row dense>
          ...
          <v-col cols="12" sm="6" md="3">
            <div class="text-caption text-uppercase text-grey-darken-1 font-weight-bold mb-1">Restocked By</div>
            <div class="text-body-1 text-capitalize">{{ item.restockedBy || 'Unknown' }}</div>
          </v-col>

          <v-col cols="12" sm="6" md="3">
            <div class="text-caption text-uppercase text-grey-darken-1 font-weight-bold mb-1">Date</div>
            <div class="text-body-1">{{ item.date || '--' }}</div>
          </v-col>

          <v-col cols="12" sm="6" md="3">
            <div class="text-caption text-uppercase text-grey-darken-1 font-weight-bold mb-1">Grand Total</div>
...
const headers = ref([
  ...
  { title: "Quantity", key: "quantity", align: "start" as const, sortable: false },
  {
    title: "Unit Cost",
    key: "unitCost",
  ...
  { title: "totalCost", key: "totalCost", align: "start" as const, sortable: false },
]);

const serverItems = ref([]);

async function fetchDetails({ page = 1, itemsPerPage }) {
  loading.value = true;
